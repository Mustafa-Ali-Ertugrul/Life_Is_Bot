...
def _event(*, interpretation_json: str) -> ReminderEvent:
    return ReminderEvent(
        ...
        related_type="habit",
        related_id=1,
        scheduled_at=datetime(2026, 8, 3, 6, 0, tzinfo=UTC),
...
async def test_generate_daily_events_creates_event(db_session: AsyncSession) -> None:
    ...
    await habit_service.create_habit(db_session, user.id, "Sabah sporu", 12, 0, "1")
    ...
    assert len(events) == 1
    assert events[0].related_type == "habit"
...
async def test_generate_daily_events_skips_other_weekdays(db_session: AsyncSession) -> None:
    user = await user_service.find_or_create_by_telegram_id(db_session, TELEGRAM_USER_ID)
    await habit_service.create_habit(db_session, user.id, "Sabah sporu", 12, 0, "2")
...
def test_event_label_habit_name() -> None:
    event = _event(interpretation_json='{"habit_name": "Sabah sporu"}')

    assert HabitModule().event_label(event) == "Sabah sporu"


def test_event_label_empty_interpretation_returns_none() -> None:
    event = _event(interpretation_json="{}")

    assert HabitModule().event_label(event) is None
