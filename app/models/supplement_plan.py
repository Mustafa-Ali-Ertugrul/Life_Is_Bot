...
class SupplementPlan(Base, TimestampMixin):
    ...
    __table_args__ = (
        Index("ix_supplement_plans_user_active", "user_id", "is_active"),
        CheckConstraint(
            "target_hour BETWEEN 0 AND 23", name="ck_supplement_plans_target_hour_range"
        ),
        CheckConstraint(
            "target_minute BETWEEN 0 AND 59", name="ck_supplement_plans_target_minute_range"
        ),
        CheckConstraint(
    ...
    user: Mapped["User"] = relationship(back_populates="supplement_plans")


__all__ = ["SupplementPlan"]
